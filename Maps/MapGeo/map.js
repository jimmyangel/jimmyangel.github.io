// Warn use if browser not supported

$(function() { $.reject({
	reject: {
		msie: 9
	},
	header: "Unfortunately your browser is not supported here",
	paragraph1: "You are currently using an unsupported browser",
	paragraph2: "Please upgrade or install one of these browsers"
	});
	return false;
});
...
function initmap() {
	...
	layerControl = L.control.layers(baseLayers, null, {position: "topleft", collapsed: true}).addTo(map);
	...
	var trackLayer = omnivore.gpx('tracks/AbelTasmanWaterTaxi.GPX', null, customLayer).on('ready', function() {
	...
	var tl = omnivore.gpx('tracks/AbelTasman.GPX', null, customLayer).on('ready', function() {
	...
	for (k=0; k<photoPos.length; k++) {
		var img ='<a href="photos/pic' + (k+1) + '.jpg" ' + 'data-lightbox="image-1" data-title="' + photoPos[k][1] + '" ><img src="photos/thumb' + (k+1) + '.jpg" width="40" height="40"/></a>';
